...
def send():  # event is passed by binders.
    ...
    msg = my_msg.get('0.0',tkinter.END).replace('\n', '')
    my_msg.delete('1.0', tkinter.END)  # Clears input field.
    print('Sending: ' + msg)
...
def red(): msglist.config(fg="red")
def green(): msglist.config(fg="green")
def blue(): msglist.config(fg="blue")
...
top = tkinter.Tk()
top.title("MobileEmbedProject")
top.resizable(0,0)

# Col-1

frmA1 = tkinter.Frame(width=1, height=30)
frmA2 = tkinter.Frame(width=1, height=300)
frmA3 = tkinter.Frame(width=1, height=140)
frmA4 = tkinter.Frame(width=1, height=30)
...
send_button = tkinter.Button(button_frame,width =8, text="Send", command=send)
cancel_button = tkinter.Button(button_frame,width= 8, text="cancel", command=cancel)
...
colorMenubt.menu.add_separator()
